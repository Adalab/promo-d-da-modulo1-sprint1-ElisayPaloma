...
los 3 pedidos que han supuesto un mayor coste económico total para la empresa. 
Calcúlalo y dale el alias ImporteTotal. Nota: Utiliza unit_price y quantity para calcular el importe total.*/

SELECT product_name, unit_price * quantity_per_unit AS ImporteTotal
FROM products
ORDER BY unit_price DESC
LIMIT 3;

/* 6. Los pedidos que están entre las posiciones 5 y 10 de nuestro ranking:
Ahora, no sabemos bien por qué razón, desde el departamento de Ventas nos piden seleccionar el ID de los pedidos 
situados entre la 5 y la 10 mejor posición en cuanto al coste económico total ImporteTotal.*/

SELECT product_id, product_name, unit_price * quantity_per_unit AS ImporteTotal
FROM products
ORDER BY unit_price * quantity_per_unit DESC
LIMIT 5
OFFSET 5; 
...
parece ser el de aquellos con un precio mayor o igual a 15 dólares, pero menor o igual que 50 dólares.
