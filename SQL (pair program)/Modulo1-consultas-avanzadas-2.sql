...
Adicionalmente nos piden que diseñemos otra consulta para conocer el número 
de productos y el precio medio de todos ellos (en general, no por cada producto).*/

SELECT COUNT(product_id), AVG(unit_price)
FROM products;

/* 3. Sacad la máxima y mínima carga de los pedidos de UK:
Nuestro siguiente encargo consiste en preparar una consulta que devuelva la máxima 
y mínima cantidad de carga para un pedido (freight) enviado a Reino Unido (United Kingdom)..*/
...
desde el departamento de Ventas quieren conocer qué productos en concreto 
se venden por encima del precio medio para todos los productos de la empresa, 
...
# Sacamos el precio medio
SELECT AVG(unit_price) AS `precio medio`
FROM products;

# Utilizamos la funcion where para pedirle solo los productos con un precio mayor del precio medio
SELECT product_name
FROM products
WHERE unit_price > 28.866363636363637;

/* 5. Qué productos se han descontinuado:
De cara a estudiar el histórico de la empresa nos piden una consulta para 
conocer el número de productos que se han descontinuado. 
El atributo Discontinued es un booleano: si es igual a 1 el 
...
No nos pueden decir del departamento si habrá pocos o muchos resultados, 
